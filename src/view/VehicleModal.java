...
public class VehicleModal implements ViewModule {
	...
	/**
	 * Submits the information from modal
	 * 
	 * @param e
	 */
		...
		if (validateFields(reg, model, brand, distance, cargo)) {
			...
			if (vehicle == null) {
				controller.createVehicle(
						new Vehicle(reg, drivenDistance, maxCargoLoad, model, brand, maintainedCheckBox.isSelected()));
			} else {
	...
	/**
	 * Tries to parse the distanceTextField input
	 * 
	 * @return bool
	 */
	...
	/**
	 * Tries to parse the cargoTextField input
	 * 
	 * @return bool
	 */
	...
	/**
	 * Validates inputs from textfields
	 * 
	 * @param reg
	 * @param model
	 * @param brand
	 ...
	 * @return
	 */
	public static VehicleModal createEditVehicleModal(IController controller, SubmitObserver observer,
			Vehicle vehicle) {
